...
class UfmfV1(UfmfBase):
    ...
    def __init__(
        self, filename, seek_ok=True, use_conventional_named_mean_fmf=True,
    ):
...
class _UFmfV3LowLevelReader(object):
    ...
    def _read_keyframe_chunk(self):
        ...
        if dtype_char == b"B":
            ...
            raise ValueError(
                'unknown dtype char "%s" (0x%0x)' % (dtype_char, ord(dtype_char))
            )
    ...
    def _read_frame_chunk(self):
        ...
        """
        intup = struct.unpack(
            FMT[self._version].POINTS1, self._fd_read(self._points1_sz)
        )
        timestamp, n_pts = intup
        ...
        for ptno in range(n_pts):
            intup = struct.unpack(
                FMT[self._version].POINTS2, self._fd_read(self._points2_sz)
            )
            (xmin, ymin, w, h) = intup
            lenbuf = w * h
        ...
        if len(buf) != n_bytes:
            if not short_OK:
                raise ShortUFMFFileError(
                    "expected %d bytes, got %d: short file %s?"
...
class _UFmfV3Indexer(object):
    ...
    def _create_index(self):
        self._index = {
            "keyframe": collections.defaultdict(dict),
            "frame": dict(timestamp=[], loc=[]),
        }
    ...
    def _index_next_chunk(self):
        ...
        try:
            if chunk_id == KEYFRAME_CHUNK:
                ...
                raise ValueError(
                    "unexpected byte %d where chunk ID expected" % (chunk_id,)
                )
        ...
        **Optional keyword arguments**
        ...
        if index_location == 0:
            ...
            try:
                index_dict_location = (
                    self._fd.tell() + 1
                )  # not the chunk location - add one
    ...
    def get_keyframe_for_timestamp(self, keyframe_type, timestamp):
        ...
        if len(idxs) == 0:
            raise NoMoreFramesException(
                "no keyframe_type %s prior to %s" % (keyframe_type, repr(timestamp))
            )
        idx = idxs[-1]
...
class FlyMovieEmulator(object):
    def __init__(
        ...
        **kwargs
        ...
        if self.abs_diff:
            if not (
                isinstance(self._ufmf, UfmfV1)
                and self._ufmf.use_conventional_named_mean_fmf
            ):
                raise NotImplementedError(
                    "abs_diff currently requires UfmfV1 "
    ...
    def get_next_frame(self, _return_more=False):
        ...
        for timestamp, regions in self._ufmf.readframes():
            if isinstance(self._ufmf, UfmfV1):
                if self._ufmf.use_conventional_named_mean_fmf:
                    tmp = self._ufmf.get_mean_for_timestamp(
                        timestamp, _return_more=_return_more
                    )
                    if _return_more:
                ...
                try:
                    mean_image, im_timestamp = self._ufmf.get_keyframe_for_timestamp(
                        "mean", timestamp
                    )
                    ...
                    mean_image.fill(255)
                if _return_more:
                    sumsqf_image, sq_timestamp = self._ufmf.get_keyframe_for_timestamp(
                        "sumsq", timestamp
                    )
                    more["sumsqf"] = sumsqf_image
            ...
            for xmin, ymin, bufim in regions:
                h, w = bufim.shape
                self._last_frame[ymin : ymin + h, xmin : xmin + w] = np.clip(
                    bufim - self._darken, 0, 255
                )
            if self.abs_diff:
                self._last_frame = abs(
                    self._last_frame.astype(np.float32) - mean_image.astype(np.float32)
                )
    ...
    def _fill_timestamps_and_locs(self):
        ...
        try:
            ...
            if int(os.environ.get("UFMF_FORCE_CACHE", "0")):
                ...
                warnings.warn(
                    str(err) + " (set environment variable "
